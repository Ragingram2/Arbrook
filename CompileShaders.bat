...
for %%i in (%sourcePath%\*.sc) do (
    ...
    "resources\tools\shader compiler\shaderc.exe" -f %sourcePath%\%%~ni.sc -p %dx11Profile% --type fragment -o %destinationPathDX11%\%%~ni.shader --varyingdef %includePath%\varying.def.sc --v -i %includePath%
    )
    ...
    "resources\tools\shader compiler\shaderc.exe" -f %sourcePath%\%%~ni.sc -p %dx11Profile% --type vertex -o %destinationPathDX11%\%%~ni.shader --varyingdef %includePath%\varying.def.sc --v -i %includePath%
    )
)
ENDLOCAL
pause
